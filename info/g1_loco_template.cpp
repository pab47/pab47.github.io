...
int main(int argc, char const *argv[]) {
  ...
  for (const auto &arg_pair : args) 
  {
    ...
    //set to 500 or 801 (only those two are supported)
    ...
    //setting to 0 is stand still / setting to 1 is stepping in place (same as conintous gait)
    ...
    //works need to be in fsm_id = 500 (start)
    ...
    //put in damp mode, emulates L2+B
    ...
    //puts in squat mode, no balance control
    if (arg_pair.first == "squat") {
      client.Squat();
    }

    //puts in sit mode with no balance
    if (arg_pair.first == "sit") {
      client.Sit();
    }

    //puts in stand up mode, no balane control (this emulates L2+up on the joystick)
    if (arg_pair.first == "stand_up") {
      client.StandUp();
    }

    //puts in zero torque mode, emulates L2+Y
    ...
    //but first set --start which will get to fsm=500 or set_fsm_id=500
    ...
    //couldnt get this to work / did not try
