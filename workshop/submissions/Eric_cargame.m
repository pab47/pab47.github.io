...
grid on;
hold on;
title(['directon change <left/right>; speed change <up/down> ; level = ',num2str(level)],'Fontsize',14);
...
patch(obstacle_x4+obstacle_R4*cos(phi),obstacle_y4+obstacle_R4*sin(phi),'y');
patch(obstacle_x5+obstacle_R5*cos(phi),obstacle_y5+obstacle_R5*sin(phi),'g');
...
if (dist_robot_obstacle1 <= (obstacle_R1+R) )
